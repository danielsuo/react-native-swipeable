...
export default class Swipeable extends PureComponent {
  ...
  _clearPress = () => {
    if (!this._pressActivatedTimeout) {
      this.props.onPressOut();
    }
    clearTimeout(this._pressActivatedTimeout);
    this._pressActivatedTimeout = null;
    this.setState({ pressActivated: false });
    this._pressActivated = false;
  }
  ...
  _handlePanResponderStart = (event, gestureState) => {
    ...
    if (!this._disablePress) {
      this._pressActivated = true;
      this._pressActivatedTimeout = setTimeout(() => {
        this.setState({ pressActivated: true });
  ...
  _handlePanResponderMove = (event, gestureState) => {
    ...
    if (this.props.disabled && !this._isSwiping) {
      this._clearPress();
    ...
    if (Math.abs(dx) > this.props.swipeStartMinDistance) {
  ...
  _handlePanResponderEnd = (event, gestureState) => {
    ...
    if (!this.props.disabled && this._pressActivated) {
      this.props.onPress();
      this._clearPress();
    }
  ...
  _panResponder = PanResponder.create({
    ...
    onPanResponderTerminationRequest: this._handlePanResponderTerminate
  });
  ...
  render() {
    ...
    return (
      <View onLayout={this._handleLayout} style={[styles.container, style, this.state.pressActivated && {backgroundColor: "rgba(0, 0, 0, 0.01)"}]} {...this._panResponder.panHandlers} {...props}>
        {canSwipeRight && (
